...
def main(_):
    ...
    if FLAGS.job_name == 'ps':
        ...
        with tf.device(tf.train.replica_device_setter(worker_device='/job:worker/task:%d' % FLAGS.task_index,
            ...
            global_step = tf.Variable(0, name='global_step', trainable=False, dtype='int32')
            ...
            summary_hook = tf.train.SummarySaverHook(save_steps=steps_to_validate,
                                                     summary_op=summary_op)

            hook.extend([summary_hook])
            ...
            with tf.train.MonitoredTrainingSession(master=server.target,
                                                   ...
                                                   hooks=hook) as sess:
                step = 0
                while step <= num_iters + sess.run(global_step):
                    if not sess.should_stop():
                        start = time.time()
                        _, step = sess.run([train_op, global_step],
                                           feed_dict={x: xdata[i * batch_size:(i + 1) * batch_size + 1],
                        ...
                        if is_chief and step % steps_to_validate == 0:
                            ...
                            print('step ', step, ' auc:', roc_auc_score(np.array(ydata), predictResult), ' loss:',
                                  lossResult, ' %.4fs/batch' % t)

                w_value, predictResult, lossResult = sess.run([w, predict, total_loss], feed_dict={x: xdata, y: ydata})
